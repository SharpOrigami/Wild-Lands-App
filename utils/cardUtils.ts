...
export function getFormattedEffectText(card: CardData, source: CardContext, playerDetails?: PlayerDetails): string | null {
    ...
    switch (effect.type) {
        ...
        case 'trap':
            ...
            return `Sets a ${effect.size} trap. Catches animals ${trapEffectiveness}.`;
        case 'campfire':
        ...
        case 'upgrade':
            ...
            if (effect.subtype === 'damage_negation') return `Negates one hit, +${effect.max_health || 0} Max HP.`;
...
export function isEventConsideredHostile(eventCard: CardData | null): boolean {
    ...
    // Illnesses and non-damaging environmental events are generally not trade-blocking hostile
    // However, some environmental events ARE hostile (like Rockslide, Lightning Strike)
