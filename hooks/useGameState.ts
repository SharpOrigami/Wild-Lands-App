...
export const useGameState = () => {
  ...
  useEffect(() => {
    if (gameState?.status === 'setup' && !preGeneratedAiBoss && !isPreGeneratingBoss) {
        ...
        generateAIBossForGame(_log).then(boss => {
  ...
  const applyDamageAndGetAnimation = useCallback((
    ...
    modPlayer.hatDamageNegationUsedThisTurn = false; // Reset before check

    if (sourceName.toLowerCase().includes('bandit')) { // Specific logging for Bandit
        _log(`[Bandit Damage Debug] Initial damage: ${damage}. Player HP before: ${modPlayer.health}. Hat available: ${modPlayer.hatDamageNegationAvailable}`, 'debug');
    }

    if (modPlayer.hatDamageNegationAvailable) {
        _log(`${modPlayer.name}'s hat deflects the blow from ${sourceName}!`, 'info');
        ...
        actualDamage = 0;
        if (sourceName.toLowerCase().includes('bandit')) _log(`[Bandit Damage Debug] Hat negated all damage.`, 'debug');
    } else {
        ...
        if (totalReduction > 0 && actualDamage < damage) {
            ...
            if (sourceName.toLowerCase().includes('bandit')) _log(`[Bandit Damage Debug] Reduction: ${totalReduction}. Damage after reduction: ${actualDamage}`, 'debug');
        }

        if (actualDamage > 0) {
            const healthBeforeDamage = modPlayer.health;
            modPlayer.health = Math.max(0, modPlayer.health - actualDamage);
            _log(`${player.name} takes ${actualDamage} damage from ${sourceName}. Health: ${modPlayer.health}/${modPlayer.maxHealth}`, 'event');
            animationDetailsReturn = { amount: actualDamage, sourceName: sourceName, eventId };
            if (sourceName.toLowerCase().includes('bandit')) _log(`[Bandit Damage Debug] Actual damage dealt: ${actualDamage}. HP before: ${healthBeforeDamage}, HP after: ${modPlayer.health}`, 'debug');
        } else if (damage > 0) {
             _log(`${player.name} takes no damage from ${sourceName}.`, 'info');
             if (sourceName.toLowerCase().includes('bandit')) _log(`[Bandit Damage Debug] No damage dealt after reductions/negation.`, 'debug');
        }
    }
    return { updatedPlayer: modPlayer, actualDamageDealt: actualDamage, animationDetails: animationDetailsReturn };
  ...
  const applyImmediateEventAndCheckEndTurn = useCallback((
      ...
      if (event.type === 'Event' &&
          ...
          const { updatedPlayer, animationDetails } = applyDamageFn(modifiablePlayer, eventEffect.amount || 0, event.name, eventId);
          ...
          if (modifiablePlayer.health <= 0) {
            ...
            return { updatedPlayer: modifiablePlayer, turnEndedByEvent, gameShouldEnd, eventRemoved, winReason, damageInfo: damageInfoForAnimation };
          }
      }
      ...
      if (eventEffect?.turn_end) { // This applies to Illnesses and Natural Threats
          ...
          if (eventId === 'threat_scarlet_fever' && eventEffect) {
              const { updatedPlayer, animationDetails } = applyDamageFn(modifiablePlayer, eventEffect.amount || 3, event.name, eventId);
              modifiablePlayer = updatedPlayer;
              damageInfoForAnimation = animationDetails;
              if (modifiablePlayer.health > 0) {
              ...
              const { updatedPlayer, animationDetails } = applyDamageFn(modifiablePlayer, damage, event.name, eventId);
              modifiablePlayer = updatedPlayer;
              damageInfoForAnimation = animationDetails;
          } else if (eventId === 'threat_rockslide' && eventEffect) {
              const { updatedPlayer, animationDetails } = applyDamageFn(modifiablePlayer, eventEffect.amount || 2, event.name, eventId);
              modifiablePlayer = updatedPlayer;
              damageInfoForAnimation = animationDetails;
              ...
              if (modifiablePlayer.health > 0 && eventEffect.discard_equipped && modifiablePlayer.equippedItems.length > 0) {
                  ...
                  if (itemsToDiscard.length > 0) {
                      ...
                      itemsToDiscard.forEach(item => {
                           modifiablePlayer.playerDiscard.push(item);
                           if (item.type === 'Player Upgrade' && item.effect?.persistent) {
                               const effect = item.effect;
                               if (effect.subtype === 'max_health' && typeof effect.amount === 'number') {
                                   const healthLoss = effect.amount; modifiablePlayer.maxHealth = Math.max(1, modifiablePlayer.maxHealth - healthLoss); modifiablePlayer.health = Math.min(modifiablePlayer.health, modifiablePlayer.maxHealth);
                               }
                               if (effect.subtype === 'damage_negation') {
                                   modifiablePlayer.hatDamageNegationAvailable = false;
                                   if (typeof effect.max_health === 'number') {
                                       const healthLoss = effect.max_health; modifiablePlayer.maxHealth = Math.max(1, modifiablePlayer.maxHealth - healthLoss); modifiablePlayer.health = Math.min(modifiablePlayer.health, modifiablePlayer.maxHealth);
                                   }
                               }
                           }
                      });
                ...
                if (damageAmount > 0) {
                    const { updatedPlayer, animationDetails } = applyDamageFn(modifiablePlayer, damageAmount, event.name, eventId);
                    modifiablePlayer = updatedPlayer;
                    damageInfoForAnimation = animationDetails;
                }
          }
...
const handleCardAction = useCallback(async (actionType: string, payload: any) => {
    ...
    if (initialTurnState.activeEvent && endTurnAttackers.some(prefix => initialTurnState.activeEvent!.id.startsWith(prefix)) && !initialTurnState.activeEventJustAttacked && (initialTurnState.activeEvent.health || 0) > 0) {
        ...
        if (!isCampfireProtectiveForAttacker || currentAttacker.subType === 'human') { // Humans attack regardless of campfire
            ...
            if (attackerEffect && attackerEffect.type === 'damage' && attackerEffect.amount) {
                const { updatedPlayer, animationDetails } = applyDamageAndGetAnimation(modifiablePlayer, attackerEffect.amount, currentAttacker.name, currentAttacker.id);
                modifiablePlayer = updatedPlayer;
    ...
    if (!gameShouldEnd) {
        ...
        if (!currentActiveEventForNewDay) { 
            ...
            if (modifiablePlayer.activeTrap && currentEventDeck.length > 0) {
                ...
                if (potentialEvent.type === 'Event' && potentialEvent.subType === 'animal') { // Ensure it's an Event card and an animal
                    let caught = false;
                    const animalHealth = potentialEvent.health || 0;
                    ...
                    if (animalHealth <= trapThreshold) caught = true;
                    ...
                    if (caught) {
                        ...
                        triggerAnimation('event-trapped-small', 'threat'); // 'threat' is fine for event card slot
                        ...
                        currentActiveEventForNewDay = null; // Slot is clear after trap
                    } else {
                        _log(`${potentialEvent.name} is too strong or unsuitable and breaks/avoids the ${trap.name}!`, 'event');
                    }
                } else if (potentialEvent.type === 'Event' && potentialEvent.subType === 'human' && (trap.effect?.size === 'medium' || trap.effect?.size === 'large')){
                    _log(`${potentialEvent.name} spots and breaks the ${trap.name}!`, 'event');
                } else {
                     _log(`The ${trap.name} is not triggered by ${potentialEvent.name}.`, 'info');
                }
                modifiablePlayer.playerDiscard.push(trap); // Trap is used up
            ...
            if (currentActiveEventForNewDay === null && !modifiablePlayer.isCampfireActive) {
                ...
                if (currentEventDeck.length > 0) {
                    ...
                    modifiableGameStateUpdates.activeEvent = newEvent; // Set new event as active
                    currentActiveEventForNewDay = newEvent; // Update for subsequent logic within this block

                    const { updatedPlayer: playerAfterImmediateEvent, turnEndedByEvent, gameShouldEnd: endAfterImmediate, eventRemoved, winReason: reasonAfterImmediate, damageInfo: immediateDamageInfo } = applyImmediateEventAndCheckEndTurn(newEvent, modifiablePlayer, _log, applyDamageAndGetAnimation);
                    ...
                    if (eventRemoved) {
                        ...
                        modifiableGameStateUpdates.activeEvent = null;
                        currentActiveEventForNewDay = null; // Reflect that it was removed
                        modifiableGameStateUpdates.activeEventTurnCounter = 0;
                    } else {
                        modifiableGameStateUpdates.activeEventTurnCounter = 1; // First turn for this new event
                    }
             ...
             // Increment its turn counter if it's the same event
            ...
            } else { // Should not happen if currentActiveEventForNewDay was from previous turn, but as safeguard
                 modifiableGameStateUpdates.activeEventTurnCounter = 1;
            }
        }

        // Start of New Day Animal Attacks (for carried-over or newly persisting animals)
        // This must happen AFTER a new event might be drawn and processed for its immediate effects.
        // `currentActiveEventForNewDay` now holds the definitive active event for the start of this new day.
        if (!gameShouldEnd && currentActiveEventForNewDay && currentActiveEventForNewDay.type === 'Event' && currentActiveEventForNewDay.subType === 'animal' && (currentActiveEventForNewDay.health || 0) > 0 && !modifiablePlayer.isCampfireActive) {
            ...
            // Use the turn counter that was just set/incremented for this event.
            // If it's a new event, its counter is 1. If carried over, it's old_counter + 1.
            ...
            if ((animal.health || 0) <= 4) { // Small animal (health <= 4)
                // Attacks ONLY IF it's the SAME animal from last turn AND player attacked it last turn.
            ...
            } else if ((animal.health || 0) > 4 && (animal.health || 0) <= 8 && !animal.id.startsWith('threat_skunk')) { // Medium animal (5-8 health), not skunk
                // Attacks at start of its SECOND turn IF it completed its first turn (i.e., its counter is now 2).
                if (effectiveTurnCounterForAttackCheck === 2) { 
                    animalShouldAttack = true;
                    _log(`${animal.name} (Medium, start of 2nd turn active) attacks!`, 'event');
                }
            } else if ((animal.health || 0) > 8) { // Large animal (health > 8)
                // Attacks IF it carried over from last turn AND player did NOT attack it last turn.
                // (Its first attack is on reveal, this is for subsequent turns).
                 if (isCarriedOverEvent && !initialTurnState.activeEventJustAttacked) {
                    animalShouldAttack = true;
            ...
            if (animalShouldAttack && animal.effect?.type === 'damage' && (animal.effect.amount || 0) > 0) {
                const { updatedPlayer, animationDetails } = applyDamageAndGetAnimation(modifiablePlayer, animal.effect.amount || 0, animal.name, animal.id);
                modifiablePlayer = updatedPlayer;
                if (animationDetails) modifiableGameStateUpdates.pendingPlayerDamageAnimation = animationDetails; // Store for animation after state update
                triggerAnimation('player-area-shake-effect', 'player');
                if (modifiablePlayer.health <= 0) {
                    gameShouldEnd = true;
  ...
  const loadInitialState = useCallback(async (isNewGamePlusContinue = false) => {
    ...
    let carriedOverDeckIds: string[] = [];
    let carriedOverEquippedIds: string[] = [];
    ...
    if (isNewGamePlusContinue) {
        ...
        try {
            carriedOverDeckIds = JSON.parse(localStorage.getItem('wildWestWinDeck_WWS') || '[]');
            carriedOverEquippedIds = JSON.parse(localStorage.getItem('wildWestEquipped_WWS') || '[]');
        } catch (e) {
            _log("Error parsing NG+ carry-over data from localStorage.", "error");
        }
        _log(`Continuing from NG+${currentNGPlusLevel}. Gold: ${carriedOverGold}, Deck Cards: ${carriedOverDeckIds.length}, Equipped: ${carriedOverEquippedIds.length}`, "system");
    } else {
         localStorage.setItem('ngPlusLevel_WWS', '0');
         localStorage.removeItem('ngPlusPlayerGold_WWS');
         localStorage.removeItem('wildWestWinDeck_WWS');
    ...
    } else if (currentNGPlusLevel > 0 && currentNGPlusLevel < NG_PLUS_PLAYER_HEALTH_BOOST_INTERVAL) {
        updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, currentNGPlusLevel));
        _log(`Applied manual NG+${currentNGPlusLevel} scaling to cards.`, "system");
    } else {
        try {
            const remixedCards = await remixCardsForNGPlusGame(_log, ALL_CARDS_DATA_MAP, currentNGPlusLevel);
            if (remixedCards) {
                 updateCurrentCardsData(remixedCards);
                 _log(`Applied AI-remixed cards for NG+${currentNGPlusLevel}.`, "system");
            } else {
                 _log(`AI remix failed for NG+${currentNGPlusLevel}, using manually scaled cards instead.`, "error");
                 updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, currentNGPlusLevel));
            }
        } catch (error) {
            console.error("Error during NG+10+ card remixing:", error);
            _log("Error applying NG+10+ card remix. Using manually scaled cards.", "error");
            updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, currentNGPlusLevel));
        }
    }
    ...
    if (!finalBossForGame || finalBossForGame.id === 'default_boss' || finalBossForGame.id === 'default_boss_fallback' || currentNGPlusLevel > 0) {
        finalBossForGame = await generateAIBossForGame(_log);
    }
    if (!finalBossForGame) finalBossForGame = await generateAIBossForGame(_log);
    ...
    // Ensure starter cards are not in the main pool for deck building to avoid duplicates if they are unique
    ...
    // availableCardsPool now contains cards not used in event deck, plus any previously excluded non-event/non-valuable/non-starter cards.
    ...
    let playerValuablesPool = availableCardsPool.filter(c => c.id.startsWith('item_gold_nugget') || c.id.startsWith('item_jewelry')); // Re-filter from remaining pool
    ...
    const itemsNeededForPlayerAugmentation = Math.max(0, (PLAYER_DECK_TARGET_SIZE - 4) - playerDeckAugmentationCards.length); // 4 is avg starter deck size
    ...
    if (savedGame) {
      try {
        ...
        if (parsedState.status && parsedState.playerDetails && parsedState.turn) {
            ...
            let cardDataToUse = ALL_CARDS_DATA_MAP;
            if (parsedState.ngPlusLevel > 0 && parsedState.ngPlusLevel < NG_PLUS_PLAYER_HEALTH_BOOST_INTERVAL) {
                cardDataToUse = applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, parsedState.ngPlusLevel);
            }

            if (parsedState.ngPlusLevel === 0) {
                 resetCurrentCardsData();
            } else if (parsedState.ngPlusLevel > 0 && parsedState.ngPlusLevel < NG_PLUS_PLAYER_HEALTH_BOOST_INTERVAL) {
                updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, parsedState.ngPlusLevel));
            } else if (parsedState.ngPlusLevel >= NG_PLUS_PLAYER_HEALTH_BOOST_INTERVAL) {
                 _log("Resuming NG+10+ game, re-applying card remix if necessary...", "debug");
                 remixCardsForNGPlusGame(_log, ALL_CARDS_DATA_MAP, parsedState.ngPlusLevel).then(remixedCards => {
                    if (remixedCards) updateCurrentCardsData(remixedCards);
                    else updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, parsedState.ngPlusLevel));
                    ...
                    updateCurrentCardsData(applyManualNGPlusScaling(ALL_CARDS_DATA_MAP, parsedState.ngPlusLevel));
                    setGameState({...parsedState, scrollAnimationPhase: parsedState.scrollAnimationPhase || 'none', pendingSkunkSprayAnimation: parsedState.pendingSkunkSprayAnimation || false });
                    _log("Resumed game from saved state (remix failed/skipped on load).", "system");
                });
                 return; // Return here because setGameState is async and we don't want to double-set
            }
            setGameState({...parsedState, scrollAnimationPhase: parsedState.scrollAnimationPhase || 'none', pendingSkunkSprayAnimation: parsedState.pendingSkunkSprayAnimation || false });
            _log("Resumed game from saved state.", "system");
            if (parsedState.aiBoss) setPreGeneratedAiBoss(parsedState.aiBoss);
             ...
             // If essential parts of the saved state are missing, treat as a fresh load
  ...
  const proceedToGamePlay = useCallback(() => {
    setGameState(prevState => {
        ...
        if (!activeEvent && eventDeck.length > 0 && !modifiablePlayer.isCampfireActive) {
            ...
            const { updatedPlayer: playerAfterImmediateEvent, turnEndedByEvent, gameShouldEnd: endAfterImmediate, eventRemoved, winReason: reasonAfterImmediate, damageInfo: immediateDamageInfo } = applyImmediateEventAndCheckEndTurn(activeEvent, modifiablePlayer, _log, applyDamageAndGetAnimation);
            modifiablePlayer = playerAfterImmediateEvent;
            ...
            if (eventRemoved) {
             ...
             // If the first event auto-ends turn (like an illness), handle the auto-end turn mechanism
             if (turnEndedByEvent && !endAfterImmediate && activeEvent?.effect?.turn_end) {
                // The banner would have been triggered in applyImmediateEventAndCheckEndTurn
                // The auto-end logic via triggerBanner's third param will handle calling endTurnLogic
        ...
        modifiableGameStateUpdates.pendingPlayerDamageAnimation = pendingDamageAnimationFromProceed || prevState.pendingPlayerDamageAnimation;
        modifiableGameStateUpdates.blockTradeDueToHostileEvent = isEventConsideredHostile(activeEvent);
        modifiableGameStateUpdates.playerDeckAugmentationPool = [];

         _log(`Game starting/resuming. Player: ${modifiablePlayer.name}, Health: ${modifiablePlayer.health}/${modifiablePlayer.maxHealth}, Deck: ${modifiablePlayer.playerDeck.length}, Hand: ${modifiablePlayer.hand.filter(c => c !== null).length} cards.`, "system");
        if (modifiablePlayer.playerDeck.length + modifiablePlayer.hand.filter(c => c !== null).length === 0 && modifiablePlayer.health > 0) {
            _log("CRITICAL ERROR: Player has no cards in deck or hand at game start!", "error");
        }
         if (modifiablePlayer.health <= 0) {
             _log("CRITICAL ERROR: Player has 0 or less health at game start!", "error");
        }
