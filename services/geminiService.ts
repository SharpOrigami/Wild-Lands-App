...
export async function generateAIBossForGame(
    ...
    playerCharacter?: Character | null, // Optional player character details
    playerName?: string | null // Optional player name
): Promise<CardData> {
  ...
  try {
    let playerContext = "";
    if (playerCharacter && playerName) {
        playerContext = `
The player character who will face this boss is named "${playerName}", a known ${playerCharacter.name}. 
Description: "${playerCharacter.storyDesc}". 
Your generated boss description should subtly hint at a past conflict, a deep-seated grudge, or a specific reason why THIS player character would be particularly driven to seek out and confront this boss. The boss's nature, motivations, or backstory should reflect this connection, making the confrontation feel personal and inevitable for ${playerName}.
`;
    }
    ...
    ${playerContext}
    Provide its name, a one-paragraph atmospheric description of its lore/appearance (this description MUST incorporate the connection to the player character if provided), its health (integer between 20 and 40), and the damage it deals (integer between 10 and 20).
